...
3.to get code from github repo to my ec2 macchine, first i have configured my username and mail is using
  git config --global user.name ""
  git config --global user.email ""

4.I did git init to intiate a local repo using git init command

5.then i mapped with my github using
  git remote add origin repourl
  
#Manual deployment without using dockerfile

6.I moved to frontend code path and performed manual deployment without using dockerfile
...
10.before using dockerfile i have deleted modules to avoid overwritten
...
12.addeed hostport 4000 in ec2 security groups

13.accessed the url with public ip and portnumber 4000 successfully

14.pushed to dockerhub using below commands
   docker login -u username
   given pat and successfully loggedin

15.before moving to dockerhub need to tag to the current image, followed the same using below commands
   docker tag frontendappv1 username/frontendappv1:1
   docker push frontendappv1 username/frontendappv1:1

16.stopped and deleted tags, images and conatiners
